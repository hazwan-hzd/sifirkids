import type { Card, ChildId, ColorKey, Profile } from "./types";

/** The three children. Order = display order on the home screen. */
export const PROFILES: Profile[] = [
  { id: "hafeeza", name: "Hafeeza", avatar: "🦋", color: "coral" },
  { id: "dhiya", name: "Dhiya", avatar: "🌟", color: "teal" },
  { id: "ilyas", name: "Ilyas", avatar: "🚀", color: "sky" },
];

/** Parent profiles - standard cards on the home screen. */
export const PARENT_PROFILES: Profile[] = [
  { id: "papa", name: "Papa", avatar: "👨‍💼", color: "grape" },
  { id: "mommy", name: "Mommy", avatar: "👩‍💼", color: "coral" },
];

export const ALL_PROFILES: Profile[] = [...PROFILES, ...PARENT_PROFILES];
export const CHILD_IDS: ChildId[] = ALL_PROFILES.map((p) => p.id);

export function getProfile(id: ChildId): Profile {
  const p = ALL_PROFILES.find((x) => x.id === id);
  if (!p) throw new Error(`Unknown child: ${id}`);
  return p;
}

/** Multiplication tables covered: 2x through 12x. */
export const TABLES: number[] = Array.from({ length: 11 }, (_, i) => i + 2); // 2..12

/* ------------------------------------------------------------------ */
/* Arabic Alif Ba Ta — 28 letters, grouped into learning sets         */
/* ------------------------------------------------------------------ */

export interface ArabicLetter {
  /** stable id used as the progress key */
  id: string;
  /** isolated-form glyph */
  glyph: string;
  /** common name */
  name: string;
  /** simple transliteration of the sound */
  sound: string;
  /** which learning set (1..4) */
  set: number;
}

export const ARABIC_LETTERS: ArabicLetter[] = [
  { id: "alif", glyph: "ا", name: "Alif", sound: "a", set: 1 },
  { id: "ba", glyph: "ب", name: "Ba", sound: "b", set: 1 },
  { id: "ta", glyph: "ت", name: "Ta", sound: "t", set: 1 },
  { id: "tha", glyph: "ث", name: "Tha", sound: "th", set: 1 },
  { id: "jim", glyph: "ج", name: "Jim", sound: "j", set: 1 },
  { id: "ha", glyph: "ح", name: "Ḥa", sound: "ḥ", set: 1 },
  { id: "kha", glyph: "خ", name: "Kha", sound: "kh", set: 1 },

  { id: "dal", glyph: "د", name: "Dal", sound: "d", set: 2 },
  { id: "dhal", glyph: "ذ", name: "Dhal", sound: "dh", set: 2 },
  { id: "ra", glyph: "ر", name: "Ra", sound: "r", set: 2 },
  { id: "zay", glyph: "ز", name: "Zay", sound: "z", set: 2 },
  { id: "sin", glyph: "س", name: "Sin", sound: "s", set: 2 },
  { id: "shin", glyph: "ش", name: "Shin", sound: "sh", set: 2 },
  { id: "sad", glyph: "ص", name: "Ṣad", sound: "ṣ", set: 2 },

  { id: "dad", glyph: "ض", name: "Ḍad", sound: "ḍ", set: 3 },
  { id: "tah", glyph: "ط", name: "Ṭa", sound: "ṭ", set: 3 },
  { id: "zah", glyph: "ظ", name: "Ẓa", sound: "ẓ", set: 3 },
  { id: "ain", glyph: "ع", name: "ʿAin", sound: "ʿa", set: 3 },
  { id: "ghain", glyph: "غ", name: "Ghain", sound: "gh", set: 3 },
  { id: "fa", glyph: "ف", name: "Fa", sound: "f", set: 3 },
  { id: "qaf", glyph: "ق", name: "Qaf", sound: "q", set: 3 },

  { id: "kaf", glyph: "ك", name: "Kaf", sound: "k", set: 4 },
  { id: "lam", glyph: "ل", name: "Lam", sound: "l", set: 4 },
  { id: "mim", glyph: "م", name: "Mim", sound: "m", set: 4 },
  { id: "nun", glyph: "ن", name: "Nun", sound: "n", set: 4 },
  { id: "hah", glyph: "ه", name: "Ha", sound: "h", set: 4 },
  { id: "waw", glyph: "و", name: "Waw", sound: "w", set: 4 },
  { id: "ya", glyph: "ي", name: "Ya", sound: "y", set: 4 },
];

export const ARABIC_SETS: number[] = [1, 2, 3, 4];

export function lettersInSet(set: number): ArabicLetter[] {
  return ARABIC_LETTERS.filter((l) => l.set === set);
}

export function getLetter(id: string): ArabicLetter | undefined {
  return ARABIC_LETTERS.find((l) => l.id === id);
}

/* ------------------------------------------------------------------ */
/* Points + mastery + rewards economy                                 */
/* ------------------------------------------------------------------ */

export const POINTS = {
  perCorrect: 13,
  /** added per question when the whole quiz is perfect */
  perfectBonusPerQuestion: 6,
  /** added once per quiz for keeping a long streak */
  streakBonus: 25,
  /** streak length that unlocks streakBonus */
  streakBonusAt: 5,
  /** bonus for keeping the daily streak alive */
  dailyStreakBonus: 19,
};

/** Harder tables earn bonus point multipliers. */
export const TABLE_MULTIPLIERS: Record<number, number> = {
  6: 2, 7: 2,   // double points
  8: 3, 9: 3,   // triple points
};

/** Mastery: at least this many attempts AND accuracy at/above threshold. */
export const MASTERY = {
  minAttempts: 10,
  accuracy: 0.9,
};

export const DEFAULT_DAILY_GOAL = 2; // quizzes per day

export interface Reward {
  id: string;
  name: string;
  icon: string;
  cost: number;
}

/** Reward shop. Parent approves claims from the dashboard. */
export const REWARDS: Reward[] = [
  { id: "sticker", name: "Sticker", icon: "✨", cost: 5000 },
  { id: "icecream", name: "Ice Cream", icon: "🍦", cost: 15000 },
  { id: "screen30", name: "30 min Screen Time", icon: "📺", cost: 20000 },
  { id: "pickdinner", name: "Pick Dinner", icon: "🍕", cost: 30000 },
  { id: "movienight", name: "Movie Night", icon: "🎬", cost: 40000 },
  { id: "stayuplate", name: "Stay Up Late", icon: "🌙", cost: 50000 },
  { id: "toy", name: "New Toy", icon: "🧸", cost: 100000 },
  { id: "outing", name: "Day Outing", icon: "🎡", cost: 200000 },
];

export const CHILD_REWARDS: Record<ChildId, Reward[]> = {
  ilyas: [
    { id: "ilyas_roblox_sticker", name: "Roblox Avatar Sticker Pack", icon: "🎟️", cost: 5000 },
    { id: "ilyas_football_kickabout", name: "Extra Football Practice", icon: "⚽", cost: 15000 },
    { id: "ilyas_robux_starter", name: "Roblox Starter Pack (400 Robux)", icon: "🪙", cost: 40000 },
    { id: "ilyas_football_socks", name: "Nike Football Boots", icon: "👟", cost: 75000 },
    { id: "ilyas_robux_mega", name: "Roblox Mega Pack (800 Robux)", icon: "💎", cost: 100000 },
    { id: "ilyas_football_jersey", name: "Custom Football Jersey", icon: "👕", cost: 50000 },
  ],
  dhiya: [
    { id: "dhiya_kawaii_stickers", name: "Kawaii Pink Unicorn Sticker Sheet", icon: "🦄", cost: 5000 },
    { id: "dhiya_pink_hairclip", name: "Pink Unicorn Hair Clip", icon: "🎀", cost: 15000 },
    { id: "dhiya_mlp_mystery_mini", name: "My Little Pony Mystery Mini Figure", icon: "🐴", cost: 35000 },
    { id: "dhiya_kawaii_skirt", name: "Pink Kawaii Tulle Skirt", icon: "👗", cost: 80000 },
    { id: "dhiya_mlp_plushie", name: "Jumbo Pinkie Pie Unicorn Plushie", icon: "🧸", cost: 120000 },
    { id: "dhiya_kawaii_room_makeover", name: "Kawaii Pink Unicorn Bedroom Set", icon: "🛏️", cost: 220000 },
  ],
  hafeeza: [
    { id: "hafeeza_kpop_photocard", name: "K-Pop Mystery Photocard Pack", icon: "📸", cost: 6000 },
    { id: "hafeeza_anime_keychain", name: "Chibi Gojo Acrylic Keychain", icon: "🔑", cost: 18000 },
    { id: "hafeeza_bakugo_poster", name: "Bakugo Katsuki Wall Poster", icon: "🖼️", cost: 30000 },
    { id: "hafeeza_gojo_blindfold", name: "Gojo Satoru Sleep Eye Mask", icon: "👁️", cost: 60000 },
    { id: "hafeeza_kpop_album", name: "K-Pop Music Album of Choice", icon: "💿", cost: 120000 },
    { id: "hafeeza_bakugo_figure", name: "My Hero Academia Bakugo Figure", icon: "🦸", cost: 250000 },
  ],
  papa: [
    { id: "papa_coffee", name: "Premium Coffee", icon: "☕", cost: 10000 },
    { id: "papa_massage", name: "Massage Voucher", icon: "💆", cost: 50000 },
    { id: "papa_gadget", name: "Tech Gadget Fund", icon: "📱", cost: 200000 },
  ],
  mommy: [
    { id: "mommy_spa", name: "Spa Day", icon: "🧖", cost: 15000 },
    { id: "mommy_flowers", name: "Fresh Flowers", icon: "💐", cost: 30000 },
    { id: "mommy_handbag", name: "Handbag Fund", icon: "👜", cost: 200000 },
  ],
};

export function getReward(id: string): Reward | undefined {
  const def = REWARDS.find((r) => r.id === id);
  if (def) return def;
  for (const list of Object.values(CHILD_REWARDS)) {
    const item = list.find((r) => r.id === id);
    if (item) return item;
  }
  return undefined;
}

/** Palette tokens per color family — used so components avoid hardcoding hex. */
export const COLOR_CLASSES: Record<
  ColorKey,
  { bg: string; bgSoft: string; text: string; ring: string; border: string; solid: string }
> = {
  coral: {
    bg: "bg-coral-500",
    bgSoft: "bg-coral-100",
    text: "text-coral-600",
    ring: "ring-coral-400",
    border: "border-coral-400",
    solid: "#ff5a47",
  },
  teal: {
    bg: "bg-teal-500",
    bgSoft: "bg-teal-100",
    text: "text-teal-600",
    ring: "ring-teal-400",
    border: "border-teal-400",
    solid: "#14c2a0",
  },
  sky: {
    bg: "bg-sky-500",
    bgSoft: "bg-sky-100",
    text: "text-sky-600",
    ring: "ring-sky-400",
    border: "border-sky-400",
    solid: "#1f9bff",
  },
  grape: {
    bg: "bg-grape-500",
    bgSoft: "bg-grape-100",
    text: "text-grape-600",
    ring: "ring-grape-400",
    border: "border-grape-400",
    solid: "#8b4dff",
  },
  sunny: {
    bg: "bg-sunny-500",
    bgSoft: "bg-sunny-100",
    text: "text-sunny-600",
    ring: "ring-sunny-400",
    border: "border-sunny-400",
    solid: "#ffba00",
  },
};

/* ------------------------------------------------------------------ */
/* SifirKids TCG: Cards & Packs Database                             */
/* ------------------------------------------------------------------ */

export const CARDS: Card[] = [
  // --- Starter Set ---
  {
    id: "starter-01",
    name: "Pikachu D. Luffy",
    emoji: "⚡👒",
    type: "lightning",
    rarity: "common",
    hp: 80,
    attackName: "Gum-Gum Thunderbolt",
    attackDmg: 30,
    description: "A sparky mouse with big dreams of becoming the Pirate King! Always hungry.",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-01.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-02",
    name: "Zoro-saur",
    emoji: "🍃⚔️",
    type: "grass",
    rarity: "common",
    hp: 90,
    attackName: "Three-Vine Leaf Slash",
    attackDmg: 35,
    description: "Carries three swords/vines in his mouth. Constantly getting lost.",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-02.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-03",
    name: "Nami Jigglypuff",
    emoji: "🫧🍊",
    type: "water",
    rarity: "common",
    hp: 70,
    attackName: "Climate Sing-Along",
    attackDmg: 20,
    description: "Predicts the weather, sings sweet lullabies, and hoards gold coins.",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-03.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-04",
    name: "Chopper Eevee",
    emoji: "🦌💊",
    type: "grass",
    rarity: "common",
    hp: 60,
    attackName: "Rumble Candy Chew",
    attackDmg: 15,
    description: "An adorable reindeer-fox that dreams of curing every disease.",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-04.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-05",
    name: "Marine Meowth",
    emoji: "🐱🪙",
    type: "marine",
    rarity: "common",
    hp: 60,
    attackName: "Justice Pay Day",
    attackDmg: 20,
    description: "Enforces absolute law while collecting shiny gold coins on patrol.",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-05.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-06",
    name: "Koby Squirtle",
    emoji: "🐢👓",
    type: "water",
    rarity: "common",
    hp: 75,
    attackName: "Honest Bubble",
    attackDmg: 25,
    description: "A very brave turtle working hard to become a Marine Admiral.",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-09.png",
    releasedIn: "SK-01",
  },

  // --- Monsters Set ---
  {
    id: "monsters-01",
    name: "Flame Luffy",
    emoji: "🔥👒",
    type: "fire",
    rarity: "uncommon",
    hp: 90,
    attackName: "Red Hawk Punch",
    attackDmg: 40,
    description: "Ignites his rubber fists with burning passion and determination.",
    set: "monsters",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-06.png",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-02",
    name: "Flame Ace Charmander",
    emoji: "🔥🎩",
    type: "fire",
    rarity: "rare",
    hp: 100,
    attackName: "Fire Fist Blast",
    attackDmg: 50,
    description: "Luffy's sworn brother. His flame tail burns hotter than magma.",
    set: "monsters",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-03",
    name: "Zoro Grovyle",
    emoji: "🐉⚔️",
    type: "grass",
    rarity: "uncommon",
    hp: 100,
    attackName: "Blade Leaf Tempest",
    attackDmg: 45,
    description: "Speeds through forests, cutting down obstacles with leaf blades.",
    set: "monsters",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-08.png",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-04",
    name: "Sanji Combusken",
    emoji: "🐓🍳",
    type: "fire",
    rarity: "uncommon",
    hp: 95,
    attackName: "Diable Kick",
    attackDmg: 45,
    description: "A fiery kick specialist who refuses to use his hands in battle.",
    set: "monsters",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-05",
    name: "Luffy Charizard",
    emoji: "🐉🔥",
    type: "fire",
    rarity: "ultra_rare",
    hp: 150,
    attackName: "Red Hawk Fireblast",
    attackDmg: 80,
    evolvesFrom: "monsters-01",
    description: "The ultimate fire dragon crew leader. Can melt solid steel.",
    set: "monsters",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-06",
    name: "Zoro Sceptile",
    emoji: "🦕🎋",
    type: "grass",
    rarity: "ultra_rare",
    hp: 140,
    attackName: "Asura Leaf Slash",
    attackDmg: 75,
    evolvesFrom: "monsters-03",
    description: "A master of the forest and swords. Can summon spectral sword vines.",
    set: "monsters",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-12.png",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-07",
    name: "Sanji Blaziken",
    emoji: "🦅🔥",
    type: "fire",
    rarity: "ultra_rare",
    hp: 140,
    attackName: "Diable Jambe Strike",
    attackDmg: 75,
    evolvesFrom: "monsters-04",
    description: "Spins at supersonic speed to ignite his legs in white-hot flames.",
    set: "monsters",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-08",
    name: "Gear 5 Pikachu",
    emoji: "⚡☁️",
    type: "legendary",
    rarity: "secret_gold",
    hp: 180,
    attackName: "Bajrang Lightning",
    attackDmg: 100,
    evolvesFrom: "starter-01",
    description: "The Sun God Nika incarnate. Can turn anything into rubber and lightning!",
    set: "monsters",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-15.png",
    releasedIn: "SK-01",
  },

  // --- Crews Set ---
  {
    id: "crews-01",
    name: "Captain Luffy",
    emoji: "🧑‍✈️🍖",
    type: "strawhat",
    rarity: "uncommon",
    hp: 100,
    attackName: "Gum-Gum Elephant Gun",
    attackDmg: 45,
    description: "Captain of the Straw Hats. Will do anything for food and friends.",
    set: "crews",
    releasedIn: "SK-01",
  },
  {
    id: "crews-02",
    name: "Pirate Hunter Zoro",
    emoji: "⚔️🟢",
    type: "strawhat",
    rarity: "uncommon",
    hp: 105,
    attackName: "1080 Pound Phoenix",
    attackDmg: 50,
    description: "First mate of the crew. His training knows no limits.",
    set: "crews",
    releasedIn: "SK-01",
  },
  {
    id: "crews-03",
    name: "Black-Leg Sanji",
    emoji: "🦵🚬",
    type: "strawhat",
    rarity: "uncommon",
    hp: 100,
    attackName: "Concasser Strike",
    attackDmg: 45,
    description: "Cook of the Straw Hats. Fights with passion and culinary pride.",
    set: "crews",
    releasedIn: "SK-01",
  },
  {
    id: "crews-04",
    name: "Fleet Admiral Akainu",
    emoji: "🌋🌋",
    type: "marine",
    rarity: "rare",
    hp: 130,
    attackName: "Magma Eruption",
    attackDmg: 65,
    description: "Leader of the Marines. Believes in absolute, unwavering justice.",
    set: "crews",
    releasedIn: "SK-01",
  },
  {
    id: "crews-05",
    name: "Admiral Aokiji Kuzan",
    emoji: "❄️🚲",
    type: "marine",
    rarity: "rare",
    hp: 120,
    attackName: "Ice Age Freeze",
    attackDmg: 60,
    description: "Loves riding his bicycle on frozen ocean waves. Chill personality.",
    set: "crews",
    releasedIn: "SK-01",
  },
  {
    id: "crews-06",
    name: "Admiral Kizaru Borsalino",
    emoji: "✨🕶️",
    type: "marine",
    rarity: "rare",
    hp: 120,
    attackName: "Yasakani Jewel",
    attackDmg: 60,
    description: "Can move at the speed of light. Speaks slowly and sarcastically.",
    set: "crews",
    releasedIn: "SK-01",
  },
  {
    id: "crews-07",
    name: "Kaido Gyarados",
    emoji: "🐉🌊",
    type: "legendary",
    rarity: "ultra_rare",
    hp: 170,
    attackName: "Bolo Breath Wave",
    attackDmg: 85,
    description: "The strongest creature alive. A massive sea dragon that commands lightning.",
    set: "crews",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-13.png",
    releasedIn: "SK-01",
  },
  {
    id: "crews-08",
    name: "Red-Haired Shanks",
    emoji: "👑🧣",
    type: "legendary",
    rarity: "secret_gold",
    hp: 160,
    attackName: "Divine Departure Haki",
    attackDmg: 95,
    description: "One of the Four Emperors. Inspires the next generation of pirates.",
    set: "crews",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-14.png",
    releasedIn: "SK-01",
  },

  // --- Promo / Mastery Set ---
  {
    id: "promo-math-01",
    name: "Sifir Seven Sparky",
    emoji: "✖️⚡",
    type: "lightning",
    rarity: "rare",
    hp: 95,
    attackName: "Table 7 Shockwave",
    attackDmg: 50,
    description: "A lightning beast unlocked by mastering the 7x Times Table.",
    set: "promo",
    releasedIn: "SK-01",
  },
  {
    id: "promo-math-02",
    name: "Multiplication Sage",
    emoji: "🧙‍♂️📖",
    type: "legendary",
    rarity: "ultra_rare",
    hp: 150,
    attackName: "Times Table Mastery",
    attackDmg: 80,
    description: "The ultimate math wizard. Uses multiplication arrays to shield allies.",
    set: "promo",
    releasedIn: "SK-01",
  },
  {
    id: "promo-arabic-01",
    name: "Alif Angel",
    emoji: "👼ا",
    type: "grass",
    rarity: "rare",
    hp: 80,
    attackName: "First Alphabet Beam",
    attackDmg: 40,
    description: "Stands straight like an Alif. Radiates clean energy and kindness.",
    set: "promo",
    releasedIn: "SK-01",
  },
  {
    id: "promo-arabic-02",
    name: "Ba Rebounder",
    emoji: "🐸ب",
    type: "water",
    rarity: "rare",
    hp: 90,
    attackName: "Letter Splash Kick",
    attackDmg: 45,
    description: "Bounces around like a frog. Unlocked by practicing Arabic letters.",
    set: "promo",
    releasedIn: "SK-01",
  },
  {
    id: "promo-arabic-03",
    name: "Tajweed Dragon",
    emoji: "🐉📖",
    type: "legendary",
    rarity: "secret_gold",
    hp: 170,
    attackName: "Holy Recitation Breath",
    attackDmg: 90,
    description: "A divine dragon protector of Quranic letters. Very wise and powerful.",
    set: "promo",
    releasedIn: "SK-01",
  },
  // --- Gojo Promotional Set ---
  {
    id: "promo-gojo-01",
    name: "Gojo-mander",
    emoji: "🔥🕶️",
    type: "fire",
    rarity: "rare",
    hp: 90,
    attackName: "Hollow Blue Flame",
    attackDmg: 45,
    description: "A cool Charmander wearing dark sunglasses. Blue flames erupt from its tail.",
    set: "promo",
    releasedIn: "SK-01",
  },
  {
    id: "promo-gojo-02",
    name: "Gojo-gar",
    emoji: "😈☁️",
    type: "shadow",
    rarity: "ultra_rare",
    hp: 130,
    attackName: "Infinite Void Smile",
    attackDmg: 70,
    description: "Gengar with spiky white hair. Smiles maniacally while trapping opponents in void.",
    set: "promo",
    releasedIn: "SK-01",
  },
  {
    id: "promo-gojo-03",
    name: "Gojo-two",
    emoji: "🧬🔴",
    type: "legendary",
    rarity: "secret_gold",
    hp: 180,
    attackName: "Hollow Purple Sphere",
    attackDmg: 100,
    description: "Mewtwo wielding the ultimate Hollow Purple. Possesses absolute psychic domain.",
    set: "promo",
    releasedIn: "SK-01",
  },

  // --- My Hero Set ---
  {
    id: "mha-01",
    name: "Izuku Midoriya",
    emoji: "🟢📓",
    type: "hero",
    rarity: "common",
    hp: 80,
    attackName: "Delaware Smash",
    attackDmg: 30,
    description: "A determined young hero who studies every move before leaping forward.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-01.png",
    releasedIn: "SK-01",
  },
  {
    id: "mha-02",
    name: "Ochaco Uraraka",
    emoji: "🪐✨",
    type: "hero",
    rarity: "common",
    hp: 65,
    attackName: "Zero Gravity Touch",
    attackDmg: 20,
    description: "A bright hero who can make heavy problems feel light.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-02.png",
    releasedIn: "SK-01",
  },
  {
    id: "mha-03",
    name: "Tenya Ida",
    emoji: "🏃👓",
    type: "hero",
    rarity: "uncommon",
    hp: 90,
    attackName: "Recipro Burst",
    attackDmg: 40,
    description: "A disciplined class representative who races ahead with engine-powered speed.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-03.png",
    releasedIn: "SK-01",
  },
  {
    id: "mha-04",
    name: "Shoto Todoroki",
    emoji: "❄️🔥",
    type: "hero",
    rarity: "rare",
    hp: 120,
    attackName: "Half-Cold Flame",
    attackDmg: 60,
    description: "A calm dual-element hero balancing ice control with blazing power.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-04.png",
    releasedIn: "SK-01",
  },
  {
    id: "mha-05",
    name: "All Might",
    emoji: "💪🌟",
    type: "hero",
    rarity: "ultra_rare",
    hp: 160,
    attackName: "Detroit Smash",
    attackDmg: 85,
    description: "The symbol of peace arrives with a heroic smile and overwhelming strength.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-05.png",
    releasedIn: "SK-02",
  },
  {
    id: "mha-06",
    name: "Bakugo Katsuki",
    emoji: "💥🦸",
    type: "hero",
    rarity: "uncommon",
    hp: 95,
    attackName: "AP Shot",
    attackDmg: 50,
    description: "Explosive palms and an explosive personality. Hafeeza's favorite!",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-06.png",
    releasedIn: "SK-01",
  },
  {
    id: "mha-07",
    name: "Tsuyu Asui",
    emoji: "🐸💚",
    type: "hero",
    rarity: "common",
    hp: 60,
    attackName: "Frog Tongue Strike",
    attackDmg: 20,
    description: "Practical and calm. Can do anything a frog can do.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-07.png",
    releasedIn: "SK-01",
  },

  // --- Squishy Squad Set ---
  {
    id: "sq-01",
    name: "Boba Blob",
    emoji: "🧋🫧",
    type: "squishy",
    rarity: "common",
    hp: 65,
    attackName: "Tapioca Bounce",
    attackDmg: 20,
    description: "A jiggly boba tea squishy that bounces back no matter how hard you squeeze.",
    set: "squishy",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/sq-01.png",
    releasedIn: "SK-01",
  },
  {
    id: "sq-02",
    name: "Donut Dough",
    emoji: "🍩💗",
    type: "squishy",
    rarity: "common",
    hp: 70,
    attackName: "Sugar Glaze Slam",
    attackDmg: 25,
    description: "A pink frosted donut squishy with sprinkles that scatter on impact.",
    set: "squishy",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/sq-02.png",
    releasedIn: "SK-01",
  },
  {
    id: "sq-03",
    name: "Panda Puff",
    emoji: "🐼☁️",
    type: "squishy",
    rarity: "uncommon",
    hp: 85,
    attackName: "Bamboo Belly Flop",
    attackDmg: 40,
    description: "So fluffy it looks like a cloud. Rolls into enemies and absorbs all damage.",
    set: "squishy",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/sq-03.png",
    releasedIn: "SK-01",
  },
  {
    id: "sq-04",
    name: "Unicorn Squish",
    emoji: "🦄🌈",
    type: "squishy",
    rarity: "uncommon",
    hp: 90,
    attackName: "Rainbow Squeeze Beam",
    attackDmg: 45,
    description: "A sparkly pastel unicorn squishy with a glitter horn. Dhiya's favourite!",
    set: "squishy",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/sq-04.png",
    releasedIn: "SK-01",
  },
  {
    id: "sq-05",
    name: "Axolotl Mega Mallow",
    emoji: "🩷🦎",
    type: "squishy",
    rarity: "rare",
    hp: 115,
    attackName: "Squishmallow Tsunami",
    attackDmg: 60,
    evolvesFrom: "sq-01",
    description: "A giant pink axolotl squishy. So soft that attacks sink in and disappear.",
    set: "squishy",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/sq-05.png",
    releasedIn: "SK-01",
  },
  {
    id: "sq-06",
    name: "Golden Galaxy Squish",
    emoji: "✨🌌",
    type: "squishy",
    rarity: "ultra_rare",
    hp: 155,
    attackName: "Cosmic Compression",
    attackDmg: 80,
    evolvesFrom: "sq-05",
    description: "The legendary golden squishy with galaxy swirl patterns. Compresses matter itself.",
    set: "squishy",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/sq-06.png",
    releasedIn: "SK-01",
  },
  {
    id: "sq-07",
    name: "Bakugo Boom Squish",
    emoji: "💥🧸",
    type: "squishy",
    rarity: "ultra_rare",
    hp: 150,
    attackName: "Explosion Squeeze",
    attackDmg: 85,
    description: "A rage-filled squishy shaped like Bakugo. Squeeze it and it EXPLODES with foam sparks!",
    set: "squishy",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/sq-07.png",
    releasedIn: "SK-01",
  },

  // --- World Cup Set ---
  {
    id: "wc-16",
    name: "Lionel Messi",
    emoji: "🇦🇷✨",
    type: "attacker",
    rarity: "ultra_rare",
    hp: 150,
    attackName: "World Cup Final Magic",
    attackDmg: 85,
    description: "The little magician turns pressure into history with one golden left foot.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-16.png",
    releasedIn: "SK-02",
  },

  // --- Jujutsu Kaisen Set (SK-02 Release) ---
  {
    id: "jjk-01",
    name: "Yuji Itadori",
    emoji: "👊💗",
    type: "sorcerer",
    rarity: "common",
    hp: 80,
    attackName: "Divergent Fist",
    attackDmg: 30,
    description: "A naturally gifted fighter hosting the King of Curses inside him.",
    set: "jjk",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/jjk-01.png",
    releasedIn: "SK-02",
  },
  {
    id: "jjk-02",
    name: "Megumi Fushiguro",
    emoji: "🐕🖤",
    type: "sorcerer",
    rarity: "common",
    hp: 75,
    attackName: "Divine Dogs",
    attackDmg: 25,
    description: "Summons shikigami from shadows. Calm and tactical.",
    set: "jjk",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/jjk-02.png",
    releasedIn: "SK-02",
  },

  // --- World Cup Set (SK-02 Release) ---
  {
    id: "wc-01",
    name: "Kylian Mbappe",
    emoji: "⚡🇫🇷",
    type: "attacker",
    rarity: "common",
    hp: 80,
    attackName: "Lightning Sprint Shot",
    attackDmg: 35,
    description: "The fastest player on the planet. Born to score World Cup goals.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-01.png",
    releasedIn: "SK-02",
  },
  {
    id: "wc-02",
    name: "Jude Bellingham",
    emoji: "🏴🎯",
    type: "midfielder",
    rarity: "common",
    hp: 78,
    attackName: "Box-to-Box Engine",
    attackDmg: 30,
    description: "England's golden boy. Covers every blade of grass.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-02.png",
    releasedIn: "SK-02",
  },
  {
    id: "wc-03",
    name: "Vinicius Jr",
    emoji: "🇧🇷💃",
    type: "attacker",
    rarity: "common",
    hp: 79,
    attackName: "Samba Stepover Finish",
    attackDmg: 30,
    description: "Brazilian flair meets clinical finishing. Dances past defenders.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-03.png",
    releasedIn: "SK-02",
  },
  {
    id: "wc-04",
    name: "Pedri",
    emoji: "🇪🇸🧠",
    type: "midfielder",
    rarity: "common",
    hp: 76,
    attackName: "La Masia Passing Lane",
    attackDmg: 25,
    description: "Barcelona academy jewel. Finds passes nobody else can see.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-04.png",
    releasedIn: "SK-02",
  },
  {
    id: "wc-05",
    name: "Gavi",
    emoji: "🇪🇸🔥",
    type: "midfielder",
    rarity: "common",
    hp: 75,
    attackName: "Press Trigger Tackle",
    attackDmg: 25,
    description: "Fearless teenager who tackles like a veteran.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-05.png",
    releasedIn: "SK-02",
  },
  {
    id: "wc-06",
    name: "Erling Haaland",
    emoji: "🇳🇴🤖",
    type: "attacker",
    rarity: "uncommon",
    hp: 91,
    attackName: "Viking Header",
    attackDmg: 50,
    description: "A goal-scoring machine. Built different.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-06.png",
    releasedIn: "SK-02",
  },
  {
    id: "wc-07",
    name: "Lamine Yamal",
    emoji: "🇪🇸⭐",
    type: "attacker",
    rarity: "uncommon",
    hp: 82,
    attackName: "Wonderkid Curler",
    attackDmg: 40,
    description: "The youngest star in football history. Curls shots into the top corner.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-07.png",
    releasedIn: "SK-03",
  },
  {
    id: "wc-08",
    name: "Virgil van Dijk",
    emoji: "🇳🇱🧱",
    type: "defender",
    rarity: "uncommon",
    hp: 88,
    attackName: "Aerial Dominance",
    attackDmg: 40,
    description: "A defensive wall. Nobody dribbles past him.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-08.png",
    releasedIn: "SK-03",
  },
  {
    id: "wc-09",
    name: "Thibaut Courtois",
    emoji: "🇧🇪🧤",
    type: "goalkeeper",
    rarity: "uncommon",
    hp: 87,
    attackName: "Reflex Save Wall",
    attackDmg: 35,
    description: "Giant keeper with cat-like reflexes. A brick wall in goal.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-09.png",
    releasedIn: "SK-03",
  },
  {
    id: "wc-10",
    name: "Cristiano Ronaldo",
    emoji: "🇵🇹🐐",
    type: "legend",
    rarity: "rare",
    hp: 92,
    attackName: "SIU Bicycle Kick",
    attackDmg: 65,
    description: "All-time top scorer. The SIU celebration is iconic.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-10.png",
    releasedIn: "SK-03",
  },
  {
    id: "wc-11",
    name: "Neymar Jr",
    emoji: "🇧🇷🌈",
    type: "attacker",
    rarity: "rare",
    hp: 88,
    attackName: "Rainbow Flick Finish",
    attackDmg: 55,
    description: "Samba magic on the pitch. Skills that defy physics.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-11.png",
    releasedIn: "SK-03",
  },
  {
    id: "wc-12",
    name: "Zinedine Zidane",
    emoji: "🇫🇷👑",
    type: "legend",
    rarity: "rare",
    hp: 95,
    attackName: "Marseille Roulette",
    attackDmg: 60,
    description: "French elegance. The roulette is the most beautiful move in football.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-12.png",
    releasedIn: "SK-03",
  },
  {
    id: "wc-13",
    name: "Ronaldo Nazario (R9)",
    emoji: "🇧🇷🦷",
    type: "legend",
    rarity: "ultra_rare",
    hp: 97,
    attackName: "El Fenomeno Sprint",
    attackDmg: 85,
    description: "The original Ronaldo. Unstoppable at his peak.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-13.png",
    releasedIn: "SK-03",
  },
  {
    id: "wc-14",
    name: "Diego Maradona",
    emoji: "🇦🇷🪄",
    type: "legend",
    rarity: "ultra_rare",
    hp: 98,
    attackName: "Hand of God / Goal of the Century",
    attackDmg: 90,
    description: "Genius and controversy in one legend. The greatest dribbler ever.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-14.png",
    releasedIn: "SK-03",
  },
  {
    id: "wc-15",
    name: "Pele",
    emoji: "🇧🇷👑",
    type: "legend",
    rarity: "secret_gold",
    hp: 99,
    attackName: "King's Bicycle Kick",
    attackDmg: 100,
    description: "The King of Football. 3x World Cup champion. The greatest ever.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-15.png",
    releasedIn: "SK-03",
  },

  // --- Alternate Art Cards (SK-01) ---
  {
    id: "starter-01-alt",
    name: "Pikachu D. Luffy (Alt Art)",
    emoji: "⚡👒✨",
    type: "lightning",
    rarity: "common",
    hp: 80,
    attackName: "Gum-Gum Thunderbolt",
    attackDmg: 30,
    description: "A sparky mouse with big dreams of becoming the Pirate King! Always hungry. [Alternate Art]",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-01-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-02-alt",
    name: "Zoro-saur (Alt Art)",
    emoji: "🍃⚔️✨",
    type: "grass",
    rarity: "common",
    hp: 90,
    attackName: "Three-Vine Leaf Slash",
    attackDmg: 35,
    description: "Carries three swords/vines in his mouth. Constantly getting lost. [Alternate Art]",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-02-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-03-alt",
    name: "Nami Jigglypuff (Alt Art)",
    emoji: "🫧🍊✨",
    type: "water",
    rarity: "common",
    hp: 70,
    attackName: "Climate Sing-Along",
    attackDmg: 20,
    description: "Predicts the weather, sings sweet lullabies, and hoards gold coins. [Alternate Art]",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-03-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-04-alt",
    name: "Chopper Eevee (Alt Art)",
    emoji: "🦌💊✨",
    type: "grass",
    rarity: "common",
    hp: 60,
    attackName: "Rumble Candy Chew",
    attackDmg: 15,
    description: "An adorable reindeer-fox that dreams of curing every disease. [Alternate Art]",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-04-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-05-alt",
    name: "Marine Meowth (Alt Art)",
    emoji: "🐱🪙✨",
    type: "marine",
    rarity: "common",
    hp: 60,
    attackName: "Justice Pay Day",
    attackDmg: 20,
    description: "Enforces absolute law while collecting shiny gold coins on patrol. [Alternate Art]",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-05-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-01-alt",
    name: "Flame Luffy (Alt Art)",
    emoji: "🔥👒✨",
    type: "fire",
    rarity: "uncommon",
    hp: 90,
    attackName: "Red Hawk Punch",
    attackDmg: 40,
    description: "Ignites his rubber fists with burning passion and determination. [Alternate Art]",
    set: "monsters",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-06-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-04-alt",
    name: "Sanji Combusken (Alt Art)",
    emoji: "🐓🍳✨",
    type: "fire",
    rarity: "uncommon",
    hp: 95,
    attackName: "Diable Kick",
    attackDmg: 45,
    description: "A fiery kick specialist who refuses to use his hands in battle. [Alternate Art]",
    set: "monsters",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-07-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "starter-06-alt",
    name: "Koby Squirtle (Alt Art)",
    emoji: "🐢👓✨",
    type: "water",
    rarity: "common",
    hp: 75,
    attackName: "Honest Bubble",
    attackDmg: 25,
    description: "A very brave turtle working hard to become a Marine Admiral. [Alternate Art]",
    set: "starter",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-09-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-02-alt",
    name: "Flame Ace Charmander (Alt Art)",
    emoji: "🔥🎩✨",
    type: "fire",
    rarity: "rare",
    hp: 100,
    attackName: "Fire Fist Blast",
    attackDmg: 50,
    description: "Luffy's sworn brother. His flame tail burns hotter than magma. [Alternate Art]",
    set: "monsters",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-10-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "monsters-05-alt",
    name: "Luffy Charizard (Alt Art)",
    emoji: "🐉🔥✨",
    type: "fire",
    rarity: "ultra_rare",
    hp: 150,
    attackName: "Red Hawk Fireblast",
    attackDmg: 80,
    evolvesFrom: "monsters-01",
    description: "The ultimate fire dragon crew leader. Can melt solid steel. [Alternate Art]",
    set: "monsters",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/poke-11-alt.png",
    releasedIn: "SK-01",
  },
  {
    id: "wc-02-alt",
    name: "Jude Bellingham (Alt Art)",
    emoji: "🏴🎯✨",
    type: "midfielder",
    rarity: "common",
    hp: 78,
    attackName: "Box-to-Box Engine",
    attackDmg: 30,
    description: "England's golden boy. Covers every blade of grass. [Alternate Art]",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-02-alt.png",
    releasedIn: "SK-02",
  },

  // --- One Piece Series (SK-04) ---
  {
    id: "op-01",
    name: "Monkey D. Luffy",
    emoji: "👒🍖",
    type: "strawhat",
    rarity: "common",
    hp: 85,
    attackName: "Gomu Gomu no Pistol",
    attackDmg: 30,
    description: "The future Pirate King. Stretchy rubber body and an iron will.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-01.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-02",
    name: "Roronoa Zoro",
    emoji: "⚔️🟢",
    type: "strawhat",
    rarity: "common",
    hp: 95,
    attackName: "Oni Giri",
    attackDmg: 35,
    description: "First mate and three-sword master. Has zero sense of direction.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-02.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-03",
    name: "Nami",
    emoji: "🍊🗺️",
    type: "strawhat",
    rarity: "common",
    hp: 65,
    attackName: "Clima-Tact Tornado",
    attackDmg: 25,
    description: "Navigator with genius weather intuition and a love for treasure.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-03.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-04",
    name: "Usopp",
    emoji: "🎯👃",
    type: "strawhat",
    rarity: "common",
    hp: 60,
    attackName: "Firebird Star",
    attackDmg: 20,
    description: "Brave warrior of the sea (self-proclaimed). Expert sniper.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-04.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-05",
    name: "Tony Tony Chopper",
    emoji: "🦌🩺",
    type: "strawhat",
    rarity: "common",
    hp: 70,
    attackName: "Heavy Point Smash",
    attackDmg: 25,
    description: "A reindeer doctor who ate the Human-Human Fruit.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-05.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-06",
    name: "Vinsmoke Sanji",
    emoji: "🦵🔥",
    type: "strawhat",
    rarity: "uncommon",
    hp: 100,
    attackName: "Ifrit Jambe",
    attackDmg: 50,
    description: "The crew's chef. Fights exclusively with devastating kicks.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-06.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-07",
    name: "Nico Robin",
    emoji: "🌸📚",
    type: "strawhat",
    rarity: "uncommon",
    hp: 80,
    attackName: "Mil Fleur Gigantesco",
    attackDmg: 40,
    description: "Archaeologist who can sprout limbs anywhere.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-07.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-08",
    name: "Smoker",
    emoji: "🌫️🏍️",
    type: "marine",
    rarity: "uncommon",
    hp: 90,
    attackName: "White Launcher",
    attackDmg: 40,
    description: "A marine captain who pursues justice through clouds of smoke.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-08.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-09",
    name: "Boa Hancock",
    emoji: "💗🐍",
    type: "warlord",
    rarity: "uncommon",
    hp: 95,
    attackName: "Mero Mero Beam",
    attackDmg: 45,
    description: "The Pirate Empress. Turns hearts to stone with her beauty.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-09.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-10",
    name: "Trafalgar Law",
    emoji: "⚔️💀",
    type: "warlord",
    rarity: "rare",
    hp: 110,
    attackName: "Room: Shambles",
    attackDmg: 55,
    description: "The Surgeon of Death. Controls space within his Room.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-10.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-11",
    name: "Fleet Admiral Akainu",
    emoji: "🌋🌋",
    type: "marine",
    rarity: "rare",
    hp: 130,
    attackName: "Meigou",
    attackDmg: 65,
    description: "Leader of the Marines. Unwavering, absolute justice.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-11.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-12",
    name: "Sabo",
    emoji: "🔥🎩",
    type: "revolutionary",
    rarity: "rare",
    hp: 120,
    attackName: "Dragon Breath Fist",
    attackDmg: 60,
    description: "Chief of Staff of the Revolutionary Army. Inherited Ace's flame.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-12.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-13",
    name: "Big Mom Charlotte",
    emoji: "🎂👹",
    type: "emperor",
    rarity: "ultra_rare",
    hp: 160,
    attackName: "Elbaf Spear",
    attackDmg: 80,
    description: "One of the Four Emperors. Controls souls and homies.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-13.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-14",
    name: "Kaido",
    emoji: "🐉⚡",
    type: "emperor",
    rarity: "ultra_rare",
    hp: 175,
    attackName: "Thunder Bagua",
    attackDmg: 90,
    description: "World's Strongest Creature. A dragon in human form.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-14.png",
    releasedIn: "SK-04",
  },
  {
    id: "op-15",
    name: "Gol D. Roger",
    emoji: "👑⚔️",
    type: "legendary",
    rarity: "secret_gold",
    hp: 200,
    attackName: "Divine Departure",
    attackDmg: 100,
    description: "The Pirate King. The one who conquered the Grand Line.",
    set: "op",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/op-15.png",
    releasedIn: "SK-04",
  },

  // --- My Hero Academia Series (SK-04 Expansion) ---
  {
    id: "mha-08",
    name: "Denki Kaminari",
    emoji: "⚡😵",
    type: "support",
    rarity: "uncommon",
    hp: 70,
    attackName: "Indiscriminate Shock",
    attackDmg: 35,
    description: "Shoots electricity but short-circuits his own brain.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-08.png",
    releasedIn: "SK-04",
  },
  {
    id: "mha-09",
    name: "Tomura Shigaraki",
    emoji: "🖐️💀",
    type: "villain",
    rarity: "uncommon",
    hp: 100,
    attackName: "Decay Touch",
    attackDmg: 50,
    description: "Five-finger touch disintegrates anything. Leader of the League of Villains.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-09.png",
    releasedIn: "SK-04",
  },
  {
    id: "mha-10",
    name: "Bakugo - Dynamight",
    emoji: "💥💥",
    type: "hero",
    rarity: "rare",
    hp: 120,
    attackName: "Howitzer Impact",
    attackDmg: 65,
    evolvesFrom: "mha-06",
    description: "Bakugo's hero name awakening. Spinning explosion at maximum output.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-10.png",
    releasedIn: "SK-04",
  },
  {
    id: "mha-11",
    name: "Endeavor",
    emoji: "🔥👊",
    type: "hero",
    rarity: "rare",
    hp: 115,
    attackName: "Prominence Burn",
    attackDmg: 60,
    description: "The current #1 hero. Atoning for his past through fire.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-11.png",
    releasedIn: "SK-04",
  },
  {
    id: "mha-12",
    name: "Dabi",
    emoji: "🔵🔥",
    type: "villain",
    rarity: "rare",
    hp: 105,
    attackName: "Cremation Blaze",
    attackDmg: 55,
    description: "Blue flames that burn hotter than any other fire quirk.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-12.png",
    releasedIn: "SK-04",
  },
  {
    id: "mha-13",
    name: "All For One",
    emoji: "🦹‍♂️💀",
    type: "villain",
    rarity: "ultra_rare",
    hp: 160,
    attackName: "Forced Quirk Activation",
    attackDmg: 85,
    description: "The ultimate villain. Steals and uses any quirk at will.",
    set: "mha",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/mha-13.png",
    releasedIn: "SK-04",
  },

  // --- World Cup Football Legends (SK-04 Expansion) ---
  {
    id: "wc-17",
    name: "Bukayo Saka",
    emoji: "🏴⚡",
    type: "attacker",
    rarity: "uncommon",
    hp: 80,
    attackName: "Cut Inside",
    attackDmg: 40,
    description: "A reliable wide attacker who builds pressure with quick footwork and steady end product.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-17.png",
    releasedIn: "SK-04",
  },
  {
    id: "wc-18",
    name: "Jamal Musiala",
    emoji: "🇩🇪🌀",
    type: "midfielder",
    rarity: "uncommon",
    hp: 75,
    attackName: "Tight Turn",
    attackDmg: 30,
    description: "Slips through midfield gaps and creates small openings without feeling overpowered.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-18.png",
    releasedIn: "SK-04",
  },
  {
    id: "wc-19",
    name: "Rodri",
    emoji: "🇪🇸🧭",
    type: "midfielder",
    rarity: "uncommon",
    hp: 85,
    attackName: "Screen Pass",
    attackDmg: 20,
    description: "A durable anchor card that stabilizes play and keeps attacks moving calmly.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-19.png",
    releasedIn: "SK-04",
  },
  {
    id: "wc-20",
    name: "Julian Alvarez",
    emoji: "🇦🇷🐝",
    type: "attacker",
    rarity: "common",
    hp: 75,
    attackName: "Chase Down",
    attackDmg: 30,
    description: "A high-energy forward who rewards aggressive, work-rate-based play.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-20.png",
    releasedIn: "SK-04",
  },
  {
    id: "wc-21",
    name: "Achraf Hakimi",
    emoji: "🇲🇦💨",
    type: "defender",
    rarity: "common",
    hp: 80,
    attackName: "Overlap Run",
    attackDmg: 30,
    description: "A fast fullback option for simple flank pressure and recovery utility.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-21.png",
    releasedIn: "SK-04",
  },
  {
    id: "wc-22",
    name: "Harry Kane",
    emoji: "🏴🎯",
    type: "attacker",
    rarity: "rare",
    hp: 90,
    attackName: "Golden Boot Finish",
    attackDmg: 60,
    description: "A clinical striker who turns half-chances into goals with calm finishing and captain-level composure.",
    set: "wc",
    imageUrl: "https://kwxieclufgfrqakzktmr.supabase.co/storage/v1/object/public/tcg-cards/wc-24.png",
    releasedIn: "SK-04",
  },
];

export interface PackConfig {
  id: string;
  name: string;
  cost: number;
  icon: string;
  cardCount: number;
  description: string;
  allowedSets: string[];
  rarityWeights: {
    common: number;
    uncommon: number;
    rare: number;
    ultra_rare: number;
    secret_gold: number;
  };
  imageUrl?: string;
}

export const PACKS: PackConfig[] = [
  {
    id: "pack-starter",
    name: "Starter Pack",
    cost: 1000,
    icon: "🎒",
    cardCount: 5,
    description: "Contains basic Pokémon/One Piece hybrid cards to start your binder.",
    allowedSets: ["starter"],
    imageUrl: "/packs/starter_pack_foil.png",
    rarityWeights: {
      common: 71,
      uncommon: 20,
      rare: 5,
      ultra_rare: 3,
      secret_gold: 1,
    },
  },
  {
    id: "pack-monsters",
    name: "Monster Evolution Pack",
    cost: 3000,
    icon: "🔥",
    cardCount: 5,
    description: "High chance of Pokémon elementals and evolutionary stage cards.",
    allowedSets: ["starter", "monsters"],
    rarityWeights: {
      common: 49,
      uncommon: 30,
      rare: 15,
      ultra_rare: 5,
      secret_gold: 1,
    },
  },
  {
    id: "pack-crews",
    name: "Pirate Crew Booster",
    cost: 3000,
    icon: "🏴‍☠️",
    cardCount: 5,
    description: "High chance of Straw Hat crew and Marine officers.",
    allowedSets: ["starter", "crews"],
    rarityWeights: {
      common: 49,
      uncommon: 30,
      rare: 15,
      ultra_rare: 5,
      secret_gold: 1,
    },
  },
  {
    id: "pack-my-hero",
    name: "My Hero Booster",
    cost: 5000,
    icon: "🦸",
    cardCount: 5,
    description: "Better odds for My Hero Academia-inspired hero cards and premium pulls.",
    allowedSets: ["mha", "promo"],
    rarityWeights: {
      common: 23,
      uncommon: 30,
      rare: 30,
      ultra_rare: 15,
      secret_gold: 2,
    },
  },
  {
    id: "pack-legendary",
    name: "Golden Legend Pack",
    cost: 8000,
    icon: "🏆",
    cardCount: 5,
    description: "Guarantees at least one Rare or better card. Chance of Secret Gold!",
    allowedSets: ["starter", "monsters", "crews", "promo", "squishy"],
    rarityWeights: {
      common: 33,
      uncommon: 30,
      rare: 25,
      ultra_rare: 10,
      secret_gold: 2,
    },
  },
  {
    id: "pack-squishy",
    name: "Squishy Squad Pack",
    cost: 2000,
    icon: "🧸",
    cardCount: 5,
    description: "Kawaii squishy cards! 80% chance of pulling from the Squishy Squad series.",
    allowedSets: ["squishy"],
    rarityWeights: {
      common: 45,
      uncommon: 30,
      rare: 15,
      ultra_rare: 8,
      secret_gold: 2,
    },
  },
  {
    id: "pack-wc",
    name: "World Cup Booster",
    cost: 4000,
    icon: "⚽",
    cardCount: 5,
    description: "SK-02 signature pack! World Cup stars and Jujutsu Kaisen crossover cards.",
    allowedSets: ["wc"],
    rarityWeights: {
      common: 40,
      uncommon: 30,
      rare: 20,
      ultra_rare: 8,
      secret_gold: 2,
    },
  },
];
