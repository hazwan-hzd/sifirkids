...
import {
  ...
  logGeografiQuiz,
  logVocabGap,
  type GeografiQuestion,
  type ChapterInfo,
  type GeografiQuizResult,
  type QuizMode,
} from "@/lib/geografi";

const ACCENT = "sky" as const; // Dhiya's Geografi color
...
export default function GeografiPage({
  ...
  const [pastResults, setPastResults] = useState<GeografiQuizResult[]>([]);
  const [loadingChapters, setLoadingChapters] = useState(true);

  // Quiz state
  const [activeChapter, setActiveChapter] = useState<number>(1);
  const [quizMode, setQuizMode] = useState<QuizMode>("quick");
  const [questions, setQuestions] = useState<GeografiQuestion[]>([]);
  ...
  const nextQuestion = useCallback(() => {
    if (currentIdx < questions.length - 1) {
      ...
      // Save to Supabase geografi tables
      logGeografiQuiz(
        {
          child_id: "dhiya",
          chapter: activeChapter,
  ...
  if (screen === "chapters") {
    return (
      <PageShell>
        {header}
        <h1 className="mb-2 text-center font-display text-3xl font-bold text-sky-600">
          ...
          <button
            ...
            className={cn(
              ...
              quizMode === "quick"
                ...
                : "bg-white/70 text-ink/60 hover:bg-sky-50",
            )}
          >
            ⚡ Kuiz Pantas
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    attempted ? "bg-sky-100" : "bg-white/85",
                  )}
                  style={{ animationDelay: `${i * 40}ms` }}
                >
                  <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-sky-500 text-2xl text-white">
                    {CHAPTER_EMOJIS[ch.chapter] ?? "📖"}
                  </span>
                  <div className="flex-1">
                    <div className="font-display text-sm font-bold text-sky-600">
                      Bab {ch.chapter}
                    </div>
                    ...
                    showResult && isCorrectOpt
                      ...
                      : showResult && isSelected && !isCorrectOpt
                        ...
                        : isSelected
                          ...
                          : "bg-white/85 text-ink hover:bg-sky-100",
                  )}
                >
                  <span
                    className={cn(
              ...
              <input
                ...
                className={cn(
                  ...
                  alreadyAnswered
                    ...
                    : "border-sky-400 bg-white focus:ring-2 focus:ring-sky-400",
                )}
              />
            </div>
          )}
        ...
        <Card className="mb-4">
          ...
          <div className="mt-2 flex gap-2">
            <input
              ...
              className="flex-1 rounded-xl border-2 border-sky-200 bg-sky-50 px-3 py-2 text-sm font-semibold text-ink outline-none placeholder:text-ink/30 focus:border-sky-400"
            />
            <button
              onClick={saveVocabGap}
              disabled={!vocabInput.trim()}
              className="tap rounded-xl bg-sky-500 px-4 py-2 text-sm font-bold text-white disabled:opacity-40"
            >
              Simpan
            </button>
          </div>
          {vocabGaps.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {vocabGaps.map((v, i) => (
                <span
                  key={i}
                  className="rounded-lg bg-sky-100 px-2 py-0.5 text-xs font-semibold text-sky-700"
                >
                  {v.word}
                </span>
              ))}
        ...
        <Card className="flex flex-col items-center gap-4 py-8 text-center animate-pop">
          ...
          {vocabGaps.length > 0 && (
            <div className="w-full rounded-2xl bg-sky-50 p-4 text-left">
              <div className="mb-2 font-display text-sm font-bold text-sky-600">
              ...
              <div className="flex flex-wrap gap-1">
                {vocabGaps.map((v, i) => (
                  <span
                    key={i}
                    className="rounded-lg bg-sky-200 px-2 py-1 text-xs font-semibold text-sky-800"
                  >
                    {v.word}
                  </span>
                ))}
          ...
          <div className="mt-2 flex w-full flex-col gap-3">
            ...
            <Link
              href={`/play/${id}`}
              role="button"
              className="btn-pop tap inline-flex min-h-[44px] items-center justify-center rounded-full bg-transparent px-5 py-3 font-display text-lg font-semibold text-sky-600"
            >
              🏠 Laman Utama
            </Link>
          </div>
