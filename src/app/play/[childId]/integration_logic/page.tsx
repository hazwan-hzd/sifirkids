...
import type { BusinessQuestion } from "@/lib/ai-specs-questions";
import { INTEGRATION_LOGIC_QUESTIONS } from "@/lib/integration-logic-questions";

const ACCENT = "teal" as const;
const MODULE_ID = "integration_logic" as const;
const MODULE_LABEL = "No-Code Integration";
const MODULE_EMOJI = "🔗";
const QUIZ_SIZE = 10;
...
export default function IntegrationLogicPage({
  params,
}: {
  ...
  const topics = getTopics(INTEGRATION_LOGIC_QUESTIONS);

  /* ── quiz lifecycle ────────────────────────────────────────────── */

  const startQuiz = useCallback((topic: string) => {
    const pool = INTEGRATION_LOGIC_QUESTIONS.filter(
      (q) => q.topic === topic,
    );
      ...
      <PageShell>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          >
            ...
            <span className="mt-2 font-display text-xs font-semibold text-ink/50">
              {INTEGRATION_LOGIC_QUESTIONS.length} Questions Total
            </span>
          </Card>
        </div>
      </PageShell>
  ...
  if (screen === "quiz") {
    ...
    return (
      <PageShell>
        ...
        {/* explanation */}
        {showExplanation && (
          <Card className="animate-slide border border-teal-100 bg-teal-50 p-5 text-left">
            <h4 className="mb-1 font-display font-bold text-teal-800">
              💡 Explanation
            </h4>
