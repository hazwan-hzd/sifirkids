...
import { LEGAL_OPS_QUESTIONS } from "@/lib/legal-ops-questions";

const ACCENT = "coral" as const;
const MODULE_ID = "legal_ops" as const;
const MODULE_LABEL = "Malaysian Labour Law";
...
export default function LegalOpsPage({
  ...
  const topics = getTopics(LEGAL_OPS_QUESTIONS);

  /* ── quiz lifecycle ────────────────────────────────────────────── */

  const startQuiz = useCallback((topic: string) => {
    const pool = LEGAL_OPS_QUESTIONS.filter((q) => q.topic === topic);
    ...
    return (
      <PageShell>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Card
            onClick={() => {
              const selected = shuffle([...LEGAL_OPS_QUESTIONS]).slice(
                0,
                QUIZ_SIZE,
              );
          ...
          >
            ...
            <span className="mt-2 font-display text-xs font-semibold text-ink/50">
              {LEGAL_OPS_QUESTIONS.length} Questions Total
            </span>
          </Card>
        </div>
      </PageShell>
  ...
  if (screen === "quiz") {
    ...
    return (
      <PageShell>
        ...
        {/* explanation */}
        {showExplanation && (
          <Card className="animate-slide border border-coral-100 bg-coral-50 p-5 text-left">
            <h4 className="mb-1 font-display font-bold text-coral-800">
              💡 Explanation
            </h4>
            <p className="text-sm leading-relaxed text-coral-950">
              {q.explanation}
            </p>
          </Card>
        )}
